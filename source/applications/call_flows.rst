...
In the Call Flow example below we have the name as Call Flow.  Make the Extension number 30 that didn't exist until now.  Feature code with a *code as *30.  Keep the context as-is with `training.fusionpbx.com`_ . Status to display which mode. Make a PIN to help secure the call flow. Make the destination label as Day Mode. Choose a sound to familiarize which mode is activated. Choose a destination for the alternative mode. Make the alternative detination label as Night Mode. Choose an alternative sound to familiarize which mode is activated. Choose a destination for the alternative mode. Lastly, enter a description to describe what this call flow does.
